...
// =================================================================================
// Install-Alpha Script for Eru CLI
// =================================================================================
// Packs the current source as Eru.Tool with a patch-bumped alpha version and
// installs it as a global dotnet tool — useful for dog-fooding in-progress work.
//
// Version logic: reads <Version> from Eru.Cli.fsproj, strips any existing suffix,
// increments the patch segment, and appends "-alpha"  (e.g. 0.7.1 -> 0.7.2-alpha).
//
// Arguments:
...
let fsprojPath = Path.Combine(rootPath, "src/Eru.Cli/Eru.Cli.fsproj")
...
// Pack
if isDryRun then
    printfn "[Dry Run] Would pack: dotnet pack src/Eru.Cli/Eru.Cli.fsproj -c Release -p:Version=%s -o %s" alphaVersion nupkgDir
else
    printfn "Packing Eru.Tool %s..." alphaVersion
    runProcess "dotnet"
        [ "pack"; "src/Eru.Cli/Eru.Cli.fsproj"
          "-c"; "Release"
...
if isDryRun then
    printfn "[Dry Run] Would uninstall: dotnet tool uninstall --global eru.tool"
else
    printfn "Uninstalling existing eru.tool (if present)..."
    tryRunProcess "dotnet" ["tool"; "uninstall"; "--global"; "eru.tool"] rootPath |> ignore

// Install from local source
if isDryRun then
    printfn "[Dry Run] Would install: dotnet tool install --global --add-source %s eru.tool --version %s" nupkgDir alphaVersion
else
    printfn "Installing eru.tool %s..." alphaVersion
    runProcess "dotnet"
        [ "tool"; "install"; "--global"
          "--add-source"; nupkgDir
          "eru.tool"
    ...
    |> ignore
    printfn ""
    printfn "Done! Installed eru.tool %s" alphaVersion
